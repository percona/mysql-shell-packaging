...
get_database(){
    ...
    if [ "x$OS" = "xrpm" ]; then
        ...
        cmake .. -DENABLE_DOWNLOADS=1 -DWITH_SSL=system -DWITH_LTO=OFF -DWITH_BOOST=$WORKDIR/boost -DWITH_ABSEIL=bundled -Dantlr4-runtime_DIR=/opt/antlr4/usr/local/lib64/cmake/antlr4-runtime -DWITH_ZLIB=bundled -DWITH_COREDUMPER=OFF -DWITH_CURL=system -DALLOW_NO_SSE42=1 -DWITH_ADMINAPI=OFF
    fi
...
build_srpm(){
    ...
    sed -i 's:%files:if [[ -f "/usr/local/lib64/libgmock.so" ]]; then cp -a /usr/local/lib64/libgmock* %{buildroot}/usr/lib/mysqlsh/; fi\n%files\n%{_prefix}/lib/mysqlsh/libprotobuf*\n%{_prefix}/lib/mysqlsh/libabsl_*\n%{_prefix}/lib/mysqlsh/libgmock*:' mysql-shell.spec
...
build_deb(){
    ...
    if [ "x$ARCH" = "xaarch64" ]; then
        sed -i "s:-DCMAKE_BUILD_TYPE:-DWITH_LTO=OFF -DCMAKE_BUILD_TYPE:" debian/rules
    fi
    if [ "x$OS_NAME" != "xbuster" ]; then
